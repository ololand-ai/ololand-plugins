...
6. **Valuation**: Read the governed DCF with `get_dcf_valuation`. Treat its valuation numerics as usable only when the same successful response contains non-empty `dcf_run_id`, `snapshot_id`, `publication_id`, `analysis_run_id`, and `eligibility_receipt_id`; otherwise record DCF as a `[gap]` and omit every DCF value, multiple, and conclusion. In explicitly requested Full DD mode only, call `run_monte_carlo_simulation(deal_id, n_simulations=10000, seed=42)` exactly once under the authority above. In every other mode, do not call it and report that no governed Monte Carlo read endpoint exists. Treat authorized Monte Carlo numerics as usable only when that exact response contains a non-empty `simulation_id`; if it does not, record Monte Carlo as a `[gap]` and omit all distribution values without retrying or using a prior run. For an identity-complete authorized result, inspect `assumption_provenance` per parameter: when `default_used` is true for ≥2 of revenue_growth / ebitda_margin / wacc / terminal_growth, present the MC output as **sensitivity analysis**, not a defended valuation distribution. Do NOT cite mean / median / P5 / P95 / VaR / CVaR numerics in the main memo body in that case — appendix only, with a one-line caveat that the distribution reflects default priors.
...
- DCF numerics require the exact `dcf_run_id` + `snapshot_id` + `publication_id` + `analysis_run_id` + `eligibility_receipt_id` lineage returned with them; missing identity means `[gap]`, not valuation evidence
- Monte Carlo numerics require the exact `simulation_id` returned with them; missing identity means `[gap]`, no retry or prior-run fallback
