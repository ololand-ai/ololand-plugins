---
name: ololand-dd-ic-memo-skeptical
description: "Use when the user asks to run the OloLand /ic-memo-skeptical workflow. Compose an IC investment memo using the skeptical tile-stitching pattern — public-facts refresh, individual engine tiles, web fetches, and explicit gap-vs-finding framing. Refuses to render a memo against a stale stored deal record."
---

<!-- Generated by scripts/generate-plugin-artifacts.py; do not edit. -->

# Codex wrapper for /ic-memo-skeptical

Follow the OloLand command instructions below when the user asks for `/ic-memo-skeptical` or the equivalent workflow in Codex.
