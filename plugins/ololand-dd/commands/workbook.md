...
argument-hint: "<deal_id|workbook_id> [list|get|create|templates|generate|extraction]"
---
...
/workbook templates                              # registered Excel template profiles
/workbook generate <deal_id> <template_id>       # DRAFT populated workbook from a template
...
6. `extraction` — call `mcp__ololand__extract_indicators_from_document(deal_id, document_id)` to extract canonical financial indicators (with cell-level citations) from an uploaded document; returns a `task_id`. Read results back with `mcp__ololand__get_canonical_extraction(extraction_id)`.
