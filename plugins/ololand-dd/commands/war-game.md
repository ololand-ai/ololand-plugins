...
2. **Separate review from execution.** For an audit, review, or read-only request, call `ask_deal_agent(deal_id, question, required_sections=["answer", "evidence", "assumptions", "open_questions"])` with the user's verbatim counterfactual, then inspect and relay `boundary_gate`, `completion_contract`, and `grader_passed`. If a pre-upgrade server rejects `required_sections`, do **not** retry `ask_deal_agent`: that compatibility rail cannot prove read-only enforcement and may expose mutating orchestration. Report the review as unavailable until the server supports the governed request; do not call `run_war_game_simulation` or infer an answer. For a request that explicitly asks to run the simulation, continue to Execution step 1.
