---
name: ololand-dd-valuation
description: "Use when the user asks to run the OloLand /valuation workflow. Run financial valuation models (DCF, LBO, Monte Carlo, Comps) with risk-adjusted assumptions from OloLand's deterministic engines."
---

<!-- Generated by scripts/generate-plugin-artifacts.py; do not edit. -->

# Codex wrapper for /valuation

Follow the OloLand command instructions below when the user asks for `/valuation` or the equivalent workflow in Codex.
