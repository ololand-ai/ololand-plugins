...
description: "Use when the user asks to run the OloLand /firm-playbook workflow. Read the firm's standing institutional playbook attached to this deal — WACC ranges, EV/Revenue and EV/EBITDA ceilings, target IRR, min MOIC, approval thresholds, preferred frameworks, custom rules. Use at the start of any deal turn that produces a number under the firm's name."
---

<!-- Generated by scripts/generate-plugin-artifacts.py; do not edit. -->

# Codex wrapper for /firm-playbook

Follow the OloLand command instructions below when the user asks for `/firm-playbook` or the equivalent workflow in Codex.
